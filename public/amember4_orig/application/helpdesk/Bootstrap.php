...
class Bootstrap_Helpdesk extends Am_Module
{
    ...
    function renderNotification()
    {
        if ($user_id = $this->getDi()->auth->getUserId()) {
            ...
            if ($cnt) return '<div class="am-info">' . ___('You have %s%d tickets%s that require your attention',
                sprintf('<a href="%s">', REL_ROOT_URL . '/helpdesk/index/p/index/index?&_member_filter_s[]=awaiting_user_response'), $cnt, '</a>') .
               '</div>';
        }
    }
    ...
    {
        $event->addReturn(___("Can operate with helpdesk module"), "helpdesk");
    }    
