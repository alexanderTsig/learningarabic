...
';
    ...
    function createForm()
    {
        ...
        $sel = $form->addMagicSelect('conditions[product]')->setLabel('Conditions');
        $cats = $pr = $oto = array();
        ...
        $options =
            ...
            + ($pr ? array('Products' => $pr) : array())
            + ($oto ? array('OTO (Downsell)' => $oto) : array());
        ...
        $form->addSelect('view[paysys_id]')->setLabel('Paysystem (optional)')
            ->loadOptions($psList);

        $fs = $form->addFieldSet()->setLabel('Offer Page Settings');

        $fs->addText('view[title]', 'size=60')->setLabel('Title');
        ...
        $fs->addAdvCheckbox('view[no_layout]')->setLabel("Avoid using standard layout\nyou have to design entire page in the 'Offer Text' field");
