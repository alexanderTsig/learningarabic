...
class UserTable extends Am_Table_WithData
{
    ...
    function syncSortOrder()
    {
        ...
        if(@count($member_fields))
        {
            ...
            {
                $x++;
                $db->query("INSERT IGNORE INTO ?_custom_fields_sort 
                    (custom_field_table, custom_field_name, sort_order) values 
                    ('user', ?, ?)",$field_name, $x);
            }
        }
    }
    ...
    {
        if(!$this->sort_order)
            $this->sort_order = $this->getDi()->db->select("SELECT custom_field_name as ARRAY_KEY,sort_order FROM ?_custom_fields_sort where custom_field_table = 'user'");
        return intval(@$this->sort_order[$a]['sort_order']) > intval(@$this->sort_order[$b]['sort_order']);
    }
}
